...
- On phones, the five theme dots collapse into a single dot showing the current theme — tapping it cycles to the next, with the current theme's name shown beside it (dot-only on the very smallest screens). The touch target grew from 10px decorative dots to ~38px, in line with mobile tap-target guidance.
