...
- **Routes:** all public GET — `/`, `/log`, `/log/{slug}`, `/study`, `/study/{slug}`, `/docs`, `/docs/{slug}`, `/docs/{slug}/coach`, plus Livewire's `/livewire-*/update` POST (checksum-signed with `APP_KEY`). Unknown slugs 404. [VERIFIED 2026-07-08]
...
- **User input paths:** Livewire component properties only — search strings and filter values on `/log`, `/study`, and `/docs` (compared server-side with `stripos`, echoed back through Blade `{{ }}` escaping), and route slugs (looked up against the content collection; unknown → 404). Client side: no `x-html` anywhere; `?step=` on coach pages is parsed as an int and clamped in Alpine. [VERIFIED 2026-07-08]
...
- **Content agents:** `/log-dev`, `/doc-dev`, `/feras-coach`, `/study-log` write Markdown that ships to production on push. The 2026-07-08 CommonMark hardening bounds the blast radius of a bad agent write to text content (no script/HTML injection possible). PcHome material, Learning Pulse results, and private career notes must never appear here (standing rules; also encoded in each skill). External link surface: study entries may carry an `artifact` URL rendered as a link — must be `https://` (seed-integrity test enforces).
- **No CI gate yet:** tests run locally (Pest, 37 tests) but nothing blocks a push that skips them. Planned hardening: GitHub Actions test gate before deploy. [CONFIRM: still pending as of 2026-07-08]
...
- 2026-07-08 — added the public `/study` section (`/study`, `/study/{slug}`; same read-only Markdown pipeline, no new input classes beyond Livewire filter props) and the `/study-log` content agent ("new content agent gets write access" trigger — assessed same day). Study `artifact` front-matter links render as external `https://` anchors with `rel="noopener"`.
